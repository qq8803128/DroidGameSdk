...
import droid.game.gradle.extension.StableXmlConfig
...
class StableXmlPlugin implements Plugin<Project> {
        ...
        project.afterEvaluate {
            ...
            if (isAppPlugin) {
                ...
                project.android.applicationVariants.each { ApplicationVariant variant ->
                    ...
                    def prName = getMergeResourcesTaskName(variant,scope)//scope.getProcessResourcesTask().name
                    ...
                    if (!inXmlPath || !(inXmlFile = new File(inXmlPath)).exists()) {
                        ...
                        if (enableAapt2) {
                            ...
                            def mrName = getMergeResourcesTaskName(variant,scope)
