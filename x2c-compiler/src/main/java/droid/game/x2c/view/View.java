package droid.game.x2c.view;


import droid.game.x2c.compiler.Log;
import droid.game.x2c.compiler.xml.LayoutManager;
import droid.game.x2c.compiler.xml.Style;
...
public class View implements ITranslator {
    ...
    public View(String packageName, String name, Attributes attributes) {
        ...
        this.mImports.add("droid.game.x2c.X2CUtils");
        this.mImports.add(String.format("%s.R3", mPackageName));
        this.mImports.add("droid.game.common.util.UIResourcesHelper");
    ...
    private String generateView(Attributes attributes) {
        ...
        if (mParent == null) {
            stringBuilder.append("\tContext context = cloneInContext == null ? ctx : cloneInContext;\n");
            stringBuilder.append("Resources res = cloneInContext == null ? ctx.getResources() : cloneInContext.getResources();\n\n");
        }
        String obj = getObjName();
        if (mTagName.equals("include")) {
            String javaName = LayoutManager.instance().translate(getIncludeLayout());
            stringBuilder.append(String.format("%s %s =(View) new %s().createView(ctx,cloneInContext);\n"
                    , mName, obj, javaName));
        } else {
    ...
    public boolean translate(StringBuilder stringBuilder, String key, String value) {
        switch (key) {
            ...
            case "android:textColorHint":
                return setHintTextColor(stringBuilder, value);
            case "android:text":
                return setText(stringBuilder, value);
            case "android:hint":
                return setHint(stringBuilder, value);
            ...
            case "android:inputType":
                return setInputType(stringBuilder, value);
            ...
            case "android:indeterminateDrawable":
                setIndeterminateDrawable(stringBuilder,value);
                return true;
            case "android:rotation":
                setRotation(stringBuilder,value);
                return true;
            default:
                return false;
        }
    ...
    private void setRotation(StringBuilder stringBuilder, String value){
        stringBuilder.append(String.format("%s.setRotation(%sf);",getObjName(),value));
    }
    ...
    private void setIndeterminateDrawable(StringBuilder stringBuilder, String value){
        stringBuilder.append(
                String.format("%s.setIndeterminateDrawable(res.getDrawable(UIResourcesHelper.getIdentifier(context,R3.drawable.%s)));\n" +
    ...
    private boolean setInputType(StringBuilder stringBuilder, String value) {
        final int TYPE_CLASS_TEXT = 0x00000001;
        final int TYPE_TEXT_VARIATION_PASSWORD = 0x00000080;
        final int TYPE_CLASS_NUMBER = 0x00000002;
        final int TYPE_NUMBER_VARIATION_NORMAL = 0x00000000;
        final int TYPE_TEXT_VARIATION_EMAIL_ADDRESS = 0x00000020;
        mImports.add("android.text.InputType");
        int type;
        switch (value) {
            case "number":
                type = TYPE_CLASS_NUMBER | TYPE_NUMBER_VARIATION_NORMAL;
                break;
            case "textPassword":
        ...
        stringBuilder.append(String.format("%s.setInputType(%s);\n", getObjName(), type + ""));
        return true;
    }
    ...
    private boolean setHintTextColor(StringBuilder stringBuilder, String value) {
        stringBuilder.append(String.format("%s.setHintTextColor(%s);\n", getObjName(), getColor(value)));
        return true;
    }
    ...
    private boolean setHint(StringBuilder stringBuilder, String value) {
        stringBuilder.append(String.format("%s.setHint(%s);\n", getObjName(), getString(value)));
        return true;
    }
    ...
    private boolean setTextSize(StringBuilder stringBuilder, String value) {
        ...
        if (value.startsWith("@")) {
            ...
            dim = String.format("res.getDimension(IResourcesHelper.getIdentifier(context,R3.dimen.%s))", value.substring(value.indexOf("/") + 1));
        } else {
        ...
        stringBuilder.append(String.format("%s.setTextSize(%s,%sf);\n", getObjName(), unit, dim));
...
//            stringBuilder.append("}\n");
            ...
            stringBuilder.append(String.format("%s.setImageDrawable(res.getDrawable(UIResourcesHelper.getIdentifier(context,%s)));\n", getObjName(), getDrawable(value)));
        }
        return true;
    }
    ...
    private boolean setBackground(StringBuilder stringBuilder, String value) {
        if (value.startsWith("#") || value.startsWith("@color") || value.startsWith("@android:color")) {
            ...
            stringBuilder.append(String.format("%s.setBackgroundDrawable(res.getDrawable(UIResourcesHelper.getIdentifier(context,%s)));\n", getObjName(), getDrawable(value)));
        }
        return true;
    }
    ...
    private boolean setId(StringBuilder stringBuilder, String value) {
        if (value.startsWith("@android:id")) {
            ...
            mId = "R3.id." + value.substring(value.indexOf("/") + 1);
            stringBuilder.append(String.format("%s.setId(UIResourcesHelper.getIdentifier(context,%s));\n", getObjName(), mId));
        }
        return true;
    }
    ...
    public static String getDimen(String value) {
        if (value.startsWith("@dimen")) {
            ...
            return String.format("(int)res.getDimension(UIResourcesHelper.getIdentifier(context,R3.dimen.%s))", value.substring(value.indexOf("/") + 1));
    ...
    public static String getColor(String value) {
        if (value.equals("#000")) {
            ...
            return "ctx.getResources().getColor(android.R.color." + value.substring(value.indexOf("/") + 1) + ")";
        } else if (value.startsWith("@color")) {
            return "res.getColor(UIResourcesHelper.getIdentifier(context,R3.color." + value.substring(value.indexOf("/") + 1) + "))";
        } else if (value.startsWith("@drawable")) {
            return "res.getColor(UIResourcesHelper.getIdentifier(context,R3.drawable." + value.substring(value.indexOf("/") + 1) + "))";
        } else {
            return "0";
        }
    }
    ...
    public static String getString(String value) {
        if (value.startsWith("@string")) {
            return "res.getString(UIResourcesHelper.getIdentifier(context,R3.string." + value.substring(value.indexOf("/") + 1) + "))";
    ...
    public static String getDrawable(String value) {
        if (value.startsWith("@drawable")) {
            return "R3.drawable." + value.substring(value.indexOf("/") + 1);
            ...
            return "R3.mipmap." + value.substring(value.indexOf("/") + 1);
