package droid.game.x2c.compiler.xml;
...
import droid.game.x2c.compiler.Util;
...
public class MapWriter {
    ...
    public void write() {
        ...
        if (mLayouts.size() == 1 && mJavaNames.size() == 1) {
            stringBuilder.append(String.format("return new %s().createView(context,cloneInContext)", mJavaNames.get(0)));
        } else {
        ...
        MethodSpec methodSpec = MethodSpec.methodBuilder("createView")
                ...
                .addParameter(ClassName.get("android.content","Context"),"cloneInContext")
        ...
        TypeSpec typeSpec = TypeSpec.classBuilder(String.format("X2C%s_%s", mGroupId, name))
                .addSuperinterface(ClassName.get("droid.game.x2c", "IViewCreator"))
                .addModifiers(Modifier.PUBLIC)
        ...
        JavaFile javaFile = JavaFile.builder("droid.game.x2c", typeSpec)
                .addImports(imports)
                .build();
        try {
